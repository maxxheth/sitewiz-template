@props([
    'phone' => '(555) 555-5555',
    'heroTitle' => 'Attention Grabbing<br/>Header',
    'heroSubtitle' => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Quisque sagittis purus sit amet volutpat consequat.',
    'buttonText' => 'Get Started',
    'navItems' => [
        ['text' => 'Home Services', 'url' => '#'],
        ['text' => 'About Us', 'url' => '#'],
        ['text' => 'Blog Posts', 'url' => '#'],
        ['text' => 'Contact Us', 'url' => '#'],
    ],
    'images' => [
        'https://placehold.co/379x310',
        'https://placehold.co/390x310',
        'https://placehold.co/390x310',
        'https://placehold.co/379x310',
        'https://placehold.co/390x310',
        'https://placehold.co/723x310',
    ],
])

{{-- 
    This header uses Blade conventions and the color variables from ui-vars.css.
    Layout and composition closely follow the original Figma export, but with maintainable Blade and utility class structure.
    All static widths have been replaced with Tailwind responsive width classes.
--}}

<div class="relative w-full max-w-7xl mx-auto rounded-[2rem] overflow-hidden shadow" style="background: var(--surface-secondary, #212A2B); box-shadow: 0px 4px 4px var(--shadow-color-1); outline: 1px var(--border-tretinary, #4E6164) solid; outline-offset: -1px;">
    ...
    {{-- Navigation Bar --}}
    <div class="flex flex-col gap-2 pt-10 px-4 md:px-10">
        <div class="flex items-center justify-between w-full">
            {{-- Logo (SVG/Shapes as in Figma) --}}
            <div class="flex items-center gap-2">
                <div class="w-32 h-8 bg-white rounded"></div>
                <div class="flex flex-row items-center gap-0.5">
                    <div class="w-4 h-4" style="transform:rotate(180deg);background:var(--logo-color-orange,#E69D58)"></div>
                    <div class="w-4 h-4" style="transform:rotate(180deg);background:var(--logo-color-dark,#000509)"></div>
                    <div class="w-3 h-2" style="transform:rotate(180deg);background:var(--logo-color-blue,#475778)"></div>
                    <div class="w-3 h-2" style="transform:rotate(180deg);background:var(--logo-color-dark,#000509)"></div>
                    <div class="w-2 h-3" style="transform:rotate(180deg);background:var(--logo-color-green,#A0AE56)"></div>
                    <div class="w-2 h-3" style="transform:rotate(180deg);background:var(--logo-color-dark,#000509)"></div>
                    <div class="w-5 h-8" style="transform:rotate(180deg);background:var(--logo-color-green,#A0AE56)"></div>
                    <div class="w-5 h-8" style="transform:rotate(180deg);background:var(--logo-color-dark,#000509)"></div>
                    <div class="w-2 h-3" style="transform:rotate(180deg);background:var(--logo-color-dark-green,#748535)"></div>
                    <div class="w-2 h-3" style="transform:rotate(180deg);background:var(--logo-color-dark,#000509)"></div>
                </div>
            </div>
            {{-- Navigation Links --}}
            <nav class="hidden md:flex items-center gap-8 px-4 py-2 rounded-full" style="background:linear-gradient(180deg,rgba(255,255,255,0.02) 0%,rgba(255,255,255,0) 100%),var(--surface-secondary,#212A2B);box-shadow:0px 1px 1px var(--shadow-color-light-inset-1) inset;">
            ...
            {{-- Call Button --}}
            <a href="tel:{{ preg_replace('/\D/', '', $phone) }}" class="flex items-center gap-1 px-4 py-2 rounded-xl uppercase font-medium text-base shadow"
